...
// Standalone form for Mexican applicants — deliberately separate from
...
// market: "MX"; lib/validation.ts branches on that to apply the right rules.
...
const QUICK_AMOUNTS = ["5,000", "10,000", "25,000", "50,000"];

// Visible labels for the canonical EMPLOYMENT_STATUSES values.
const EMPLOYMENT_LABELS: Record<string, string> = {
  Employed: "Empleado(a) (Employed)",
  "Self-employed": "Independiente (Self-employed)",
  "Business owner": "Dueño(a) de Negocio (Business Owner)",
  Student: "Estudiante (Student)",
  Unemployed: "Desempleado(a) (Unemployed)",
  Retired: "Jubilado(a) (Retired)",
};

const STEPS = ["Sobre Ti", "Tu Préstamo", "Revisar y Enviar"];
...
export default function MexicoApplicationForm() {
  ...
  useEffect(() => {
    ...
    if (n > 0 && n <= 10_000_000) {
      setForm((f) =>
        f.amount ? f : { ...f, amount: n.toLocaleString("es-MX") }
      );
    }
  }, []);
  ...
  async function submitApplication() {
    ...
    try {
      ...
      if (res.status === 429) {
        setSubmitError(
          "Has enviado demasiadas solicitudes. Espera un minuto e inténtalo de nuevo. / Too many requests — please wait a minute and try again."
        );
      } else {
        ...
        setSubmitError(
          "No pudimos enviar tu solicitud. Verifica tus datos. / We couldn't submit your application. Please check your details."
        );
      }
    } catch {
      setSubmitError(
        "Ocurrió un error. Verifica tu conexión. / Something went wrong. Please check your connection."
      );
    }
    setSubmitting(false);
  }
  ...
  const bilingual = (es: string, en: string) => (
    <>
      {en} <span className="font-normal text-slate-400">/ {es}</span>
    </>
  );
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="mb-8">
          <div className="flex items-baseline justify-between">
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400">
              Paso {step + 1} de {STEPS.length} / Step {step + 1} of{" "}
              {STEPS.length}
            </p>
        ...
        <div key="step-0" className="animate-step-in space-y-6">
          <div>
            <label htmlFor="fullName" className={labelClass}>
              {bilingual("Nombre Completo", "Full Name")}
            </label>
            {iconField(
              "user",
              <input
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <label htmlFor="email" className={labelClass}>
                {bilingual("Correo Electrónico", "Email Address")}
              </label>
              {iconField(
                "mail",
                <input
            ...
            <div>
              <label htmlFor="phone" className={labelClass}>
                {bilingual("Número de Teléfono", "Phone Number")}
              </label>
              {iconField(
                "phone",
                <input
          ...
          <div className="grid gap-6 sm:grid-cols-[1fr_auto]">
            <div>
              <label htmlFor="address" className={labelClass}>
                {bilingual("Dirección Completa", "Full Address")}
              </label>
              {iconField(
                "pin",
                <input
            ...
            <div>
              <label htmlFor="postalCode" className={labelClass}>
                {bilingual("Código Postal", "Postal Code")}
              </label>
              <input
                id="postalCode"
                name="postalCode"
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <label htmlFor="dob" className={labelClass}>
                {bilingual("Fecha de Nacimiento", "Date of Birth")}
              </label>
              <input
                id="dob"
                name="dob"
            ...
            <div>
              <label htmlFor="education" className={labelClass}>
                {bilingual("Escolaridad", "Educational Qualification")}
              </label>
              <select
                id="education"
                name="education"
              ...
              >
                ...
                {EDUCATION_LEVELS_MX.map((level) => (
                  <option key={level} value={level}>
                    {EDUCATION_LABELS_MX[level]}
                  </option>
                ))}
              </select>
          ...
          <div>
            <label htmlFor="employment" className={labelClass}>
              {bilingual("Situación Laboral", "Employment Status")}
            </label>
            <select
              id="employment"
              name="employment"
        ...
        <div key="step-1" className="animate-step-in space-y-6">
          <div>
            <label htmlFor="amount" className={labelClass}>
              {bilingual("Monto del Préstamo (Pesos)", "Loan Amount Needed")}
            </label>
            <div className="relative">
              ...
              <input
                ...
                className={`${inputClass(!!errors.amount)} pl-9 font-display text-lg font-bold`}
              />
            </div>
          ...
          <fieldset>
            <legend className={labelClass}>
              {bilingual("Motivo del Préstamo", "Loan Purpose")}
            </legend>
          ...
          <div>
            ...
            <textarea
              ...
              name="purposeDetail"
              rows={3}
              placeholder="Cuéntanos un poco sobre el motivo de tu préstamo…"
      ...
      {step === 2 && (
        <div key="step-2" className="animate-step-in space-y-5">
          <p className="text-sm text-slate-600">
            Ya casi — revisa tus datos antes de enviar. / Almost there — check
          ...
          <section className="rounded-2xl border-2 border-slate-100 px-5 py-4">
            ...
            <dl className="mt-1 divide-y divide-slate-100">
              <ReviewRow label="Nombre / Name" value={form.fullName} />
              <ReviewRow label="Email" value={form.email} />
              <ReviewRow label="Teléfono / Phone" value={form.phone} />
              <ReviewRow label="Dirección / Address" value={form.address} />
              <ReviewRow
                label="Código Postal / Postal"
                value={form.postalCode || "—"}
              />
              <ReviewRow
          ...
          <section className="rounded-2xl border-2 border-slate-100 px-5 py-4">
            ...
            <dl className="mt-1 divide-y divide-slate-100">
              <ReviewRow label="Monto / Amount" value={`$ ${form.amount}`} />
              <ReviewRow label="Motivo / Purpose" value={form.purpose} />
              {form.purposeDetail.trim() && (
                <ReviewRow label="Detalles" value={form.purposeDetail.trim()} />
              )}
            </dl>
          </section>
        </div>
      ...
      <p className="mt-6 flex items-center justify-center gap-1.5 text-xs text-slate-400">
        ...
        Tus datos están seguros. Esta solicitud no afecta tu historial
        crediticio. / Your details are secure. Applying won&apos;t affect your
        credit score.
      </p>
    </form>
  );
