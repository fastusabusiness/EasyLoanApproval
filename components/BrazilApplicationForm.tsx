...
// Standalone form for Brazilian applicants — deliberately separate from
...
// market: "BR"; lib/validation.ts branches on that to apply the right rules.
...
const EMPLOYMENT_LABELS: Record<string, string> = {
  Employed: "Empregado(a) (Employed)",
  "Self-employed": "Autônomo(a) (Self-employed)",
  "Business owner": "Empresário(a) (Business Owner)",
  Student: "Estudante (Student)",
  Unemployed: "Desempregado(a) (Unemployed)",
  Retired: "Aposentado(a) (Retired)",
};

const STEPS = ["Sobre Você", "Seu Empréstimo", "Revisar e Enviar"];
...
export default function BrazilApplicationForm() {
  ...
  useEffect(() => {
    ...
    if (n > 0 && n <= 10_000_000) {
      setForm((f) =>
        f.amount ? f : { ...f, amount: n.toLocaleString("pt-BR") }
      );
    }
  }, []);
  ...
  async function submitApplication() {
    ...
    try {
      ...
      if (res.status === 429) {
        setSubmitError(
          "Você enviou muitos pedidos. Aguarde um minuto e tente novamente. / Too many requests — please wait a minute and try again."
        );
      } else {
        ...
        setSubmitError(
          "Não foi possível enviar seu pedido. Verifique seus dados. / We couldn't submit your application. Please check your details."
        );
      }
    } catch {
      setSubmitError(
        "Ocorreu um erro. Verifique sua conexão. / Something went wrong. Please check your connection."
      );
    }
    setSubmitting(false);
  }
  ...
  const bilingual = (pt: string, en: string) => (
    <>
      {en} <span className="font-normal text-slate-400">/ {pt}</span>
    </>
  );
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="mb-8">
          <div className="flex items-baseline justify-between">
            <p className="text-xs font-bold uppercase tracking-wider text-slate-400">
              Passo {step + 1} de {STEPS.length} / Step {step + 1} of{" "}
              {STEPS.length}
            </p>
        ...
        <div key="step-0" className="animate-step-in space-y-6">
          <div>
            <label htmlFor="fullName" className={labelClass}>
              {bilingual("Nome Completo", "Full Name")}
            </label>
            {iconField(
              "user",
              <input
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <label htmlFor="email" className={labelClass}>
                {bilingual("Endereço de Email", "Email Address")}
              </label>
              {iconField(
                "mail",
                <input
            ...
            <div>
              <label htmlFor="phone" className={labelClass}>
                {bilingual("Número de Telefone", "Phone Number")}
              </label>
              {iconField(
                "phone",
                <input
          ...
          <div className="grid gap-6 sm:grid-cols-[1fr_auto]">
            <div>
              <label htmlFor="address" className={labelClass}>
                {bilingual("Endereço Completo", "Full Address")}
              </label>
              {iconField(
                "pin",
                <input
            ...
            <div>
              ...
              <input
                ...
                className={`${baseInputClass(!!errors.postalCode)} w-full text-center font-bold sm:w-36`}
              />
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            <div>
              <label htmlFor="dob" className={labelClass}>
                {bilingual("Data de Nascimento", "Date of Birth")}
              </label>
              <input
                id="dob"
                name="dob"
            ...
            <div>
              <label htmlFor="education" className={labelClass}>
                {bilingual("Escolaridade", "Educational Qualification")}
              </label>
              <select
                id="education"
                name="education"
              ...
              >
                ...
                {EDUCATION_LEVELS_BR.map((level) => (
                  <option key={level} value={level}>
                    {EDUCATION_LABELS_BR[level]}
                  </option>
                ))}
              </select>
          ...
          <div>
            <label htmlFor="employment" className={labelClass}>
              {bilingual("Situação Profissional", "Employment Status")}
            </label>
            <select
              id="employment"
              name="employment"
        ...
        <div key="step-1" className="animate-step-in space-y-6">
          <div>
            <label htmlFor="amount" className={labelClass}>
              {bilingual("Valor do Empréstimo (Reais)", "Loan Amount Needed")}
            </label>
            <div className="relative">
          ...
          <fieldset>
            <legend className={labelClass}>
              {bilingual("Finalidade do Empréstimo", "Loan Purpose")}
            </legend>
          ...
          <div>
            <label htmlFor="purposeDetail" className={labelClass}>
              {bilingual(
                "Algo mais que devemos saber? (opcional)",
            ...
            <textarea
              ...
              name="purposeDetail"
              rows={3}
              placeholder="Conte um pouco sobre a finalidade do seu empréstimo…"
      ...
      {step === 2 && (
        <div key="step-2" className="animate-step-in space-y-5">
          <p className="text-sm text-slate-600">
            Quase lá — revise seus dados antes de enviar. / Almost there —
          ...
          <section className="rounded-2xl border-2 border-slate-100 px-5 py-4">
            ...
            <dl className="mt-1 divide-y divide-slate-100">
              <ReviewRow label="Nome / Name" value={form.fullName} />
              <ReviewRow label="Email" value={form.email} />
              <ReviewRow label="Telefone / Phone" value={form.phone} />
              <ReviewRow label="Endereço / Address" value={form.address} />
              <ReviewRow label="CEP / Postal" value={form.postalCode || "—"} />
              <ReviewRow
                label="Escolaridade / Education"
                value={form.education || "—"}
              />
              <ReviewRow
                label="Ocupação / Employment"
                value={
                  form.employment
          ...
          <section className="rounded-2xl border-2 border-slate-100 px-5 py-4">
            ...
            <dl className="mt-1 divide-y divide-slate-100">
              <ReviewRow label="Valor / Amount" value={`R$ ${form.amount}`} />
              <ReviewRow label="Finalidade / Purpose" value={form.purpose} />
              {form.purposeDetail.trim() && (
                <ReviewRow label="Detalhes" value={form.purposeDetail.trim()} />
              )}
            </dl>
          </section>
        </div>
      ...
      <p className="mt-6 flex items-center justify-center gap-1.5 text-xs text-slate-400">
        ...
        Seus dados estão seguros. Este pedido não afeta seu score de crédito. /
        Your details are secure. Applying won&apos;t affect your credit score.
      </p>
    </form>
  );
}
